# -*- coding: utf-8 -*-
"""SNES Mode-1 style explore renderer for Primal Odyssey.

Layers (like SNES Mode 1):
  BG1  — ground tilemap (16x16 metatiles)
  BG2  — tall objects (trees/rocks) as Y-sorted sprites
  OBJ  — animals + player sprites
  HUD  — existing DOM HUD
"""
# This file is JS content written via Python for encoding safety on Windows.
from pathlib import Path

js = r'''/* Primal Odyssey — SNES tilemap explore (Mode-1 style) */
(function () {
  "use strict";
  ...
  S.current = function () {
    var id = (window.region && region.id) || "africa";
  ...
  S.draw = function (ctx, W, H, player, sprites, getAnimalCanvas, dayPhase) {
    ...
    var rid = (window.region && region.id) || "africa";
...
Path(__file__).resolve().parent.joinpath("primal_snes.js").write_text(js, encoding="utf-8")
print("wrote primal_snes.js")
